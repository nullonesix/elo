    ...
    if (ent->client->ps.duelInProgress) {
        ...
        if (!duelAgainst || !duelAgainst->client || !duelAgainst->inuse || duelAgainst->client->ps.duelIndex != ent->s.number) {
            ...
            if (ent->health > 0 && ent->client->ps.stats[STAT_HEALTH] > 0) {
                ...
                // prayer's/nullonesix's and Mr. Bird's elo ranking
                // elo.txt:
                // co/prayer/n
                // 1500
                // CO:Mr. Bird:M
                ...
                // ...
                //
                // make std::string names
                std::string winner_name(winner);
                std::string loser_name(loser);
                std::map<std::string, int> d; // dictionary
                std::ifstream f("elo.txt"); // file
                std::string k; // key
                std::string v; // value
                // read file
                ...
                if(d.find(winner_name)==d.end()){
                    d[winner_name] = 1500;
                }
                if(d.find(loser_name)==d.end()){
                    d[loser_name] = 1500;
                }
                // calculate win/loss elo changes based on https://en.wikipedia.org/wiki/Elo_rating_system#Mathematical_details
                double R_A = (double) d[winner_name];
                double R_B = (double) d[loser_name];
                double E_A = 1.0/(1.0+pow(10.0, (R_B-R_A)/400.0));
                double E_B = 1.0/(1.0+pow(10.0, (R_A-R_B)/400.0));
                double K = 32.0;
                double S_A = 1.0;
                double S_B = 0.0;
                double R_A_p = R_A + K * (S_A - E_A);
                double R_B_p = R_B + K * (S_B - E_B);
                d[winner_name] = (int) R_A_p;
                d[loser_name] = (int) R_B_p;
                // compute rank
                int n_better_players_than_winner = 0;
                int n_better_players_than_loser = 0;
                for(std::map<std::string, int>::iterator it=d.begin(); it!=d.end(); it++){
                    if(it->second > d[winner_name]){
                        n_better_players_than_winner++;
                    }
                    if(it->second > d[loser_name]){
                        n_better_players_than_loser++;
                    }
                }
                // print to game
                // trap->SendServerCommand(-1, va("print \"%s " S_COLOR_WHITE "elo: %i -> %i rank: %i\n\"", winner_name.c_str(), (int) R_A, d[winner_name], n_better_players_than_winner+1));
                // trap->SendServerCommand(-1, va("print \"%s " S_COLOR_WHITE "elo: %i -> %i rank: %i\n\"", loser_name.c_str(), (int) R_B, d[loser_name], n_better_players_than_loser+1));
                // say to game
                trap->SendServerCommand(-1, va("chat \"%s " S_COLOR_WHITE "elo: " S_COLOR_YELLOW "%i" S_COLOR_WHITE " -> " S_COLOR_YELLOW "%i" S_COLOR_WHITE " rank: " S_COLOR_CYAN "%i" S_COLOR_WHITE "\n\"", winner_name.c_str(), (int) R_A, d[winner_name], n_better_players_than_winner+1));
                trap->SendServerCommand(-1, va("chat \"%s " S_COLOR_WHITE "elo: " S_COLOR_YELLOW "%i" S_COLOR_WHITE " -> " S_COLOR_YELLOW "%i" S_COLOR_WHITE " rank: " S_COLOR_CYAN "%i" S_COLOR_WHITE "\n\"", loser_name.c_str(), (int) R_B, d[loser_name], n_better_players_than_loser+1));
                // write new file
                std::ofstream g("elo.txt", std::ofstream::trunc);
                for(std::map<std::string, int>::iterator it=d.begin(); it!=d.end(); it++){
                    g << it->first << std::endl;
                    g << it->second << std::endl;
                }
            ...
            if (g_privateDuel.bits & PRIVDUEL_RESPAWN) {
                ...
                if(false){
                    ...
                    duelAgainst->client->ps.origin.z += 1 + 60;
